...
from const import USING_FULL_DATASET
...
def behavioural_cloning_train(data_dir, in_model, in_weights, out_weights):
    ...
    for batch_i, batch in enumerate(data_loader):
        ...
        for image, action, episode_id, timestep, done in zip(*batch):
            ...
            agent_action = agent._env_action_to_agent(action, to_torch=True)
            ...
            pi_distribution, _, new_agent_state, _ = policy.take_step(
                agent_obs,
                dummy_first,
                agent_state
            )
            ...
            with th.no_grad():
                original_pi_distribution, _, _, _ = original_policy.take_step(
                    agent_obs,
                    dummy_first,
                    agent_state
                )
